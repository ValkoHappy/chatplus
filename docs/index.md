...
Рабочая роль редактора в Strapi: `AI Draft Editor`. Она нужна, чтобы Алина могла без кода создавать `Page`, редактировать текст и блоки, удалять тестовые или согласованные страницы, публиковать проверенные страницы и создавать `Generation Job` для AI-доработки. Эта роль не дает редактору работу с VPS, Docker, GitHub Actions, `.env`, API tokens, Strapi schema/settings и серверными snapshot.
