...
2. Сохранить `Page`, если это новая запись.
3. В правой панели `AI Generation` нажать `Сгенерировать через AI`.
4. Strapi создаст связанную `Generation Job`, автоматически выберет текущую страницу в `target_page` и сразу откроет задачу.
5. Проверить `target_blueprint`: он должен совпадать с `page_kind` выбранной страницы.
6. В prompt описать задачу человеческим языком: тема, аудитория, цель страницы, CTA, ограничения.
7. При необходимости выбрать связи-категории: `target_channels`, `target_industries`, `target_integrations`, `target_solutions`, `target_features`, `target_business_types`, `target_competitors`.
8. Нажать кнопку запуска в `Generation Job`: runner сначала соберет AI-кандидат черновика.
9. Открыть preview кандидата, проверить текст и только потом нажать `Принять в Page`.
10. Если страница слабая, вернуться в эту же `Page`, снова нажать `Сгенерировать через AI` и написать prompt на доработку.
11. Публиковать только после человеческой проверки.

Ручной путь через `Content Manager -> Generation Job -> Create new entry` нужен только для нестандартных случаев. В обычном сценарии не нужно отдельно искать страницу в `target_page`: кнопка в `Page` делает это сама.
...
| Ситуация | Что делать |
| --- | --- |
| Нужно заполнить новую страницу с уже готовым макетом | Открыть готовую `Page`, нажать `Сгенерировать через AI`, написать prompt на заполнение |
| AI сделал черновик, но текст слабый | Открыть эту же `Page`, снова нажать `Сгенерировать через AI`, написать prompt на доработку |
...
2. В правой панели `AI Generation` нажмите `Сгенерировать через AI`.
3. Strapi создаст `Generation Job` с уже выбранной `target_page`.
4. В созданной `Generation Job` проверьте поля и напишите prompt на доработку.

Если создаете `Generation Job` вручную, заполните:
