# Аварийный/первый запуск на VPS

Этот файл **не для редактора контента**.

Используйте его только если:

- сервер чистый и сайта на нём ещё нет;
- сайт реально упал и оператор восстанавливает инфраструктуру;
- нужно переустановить Docker/nginx/Strapi/Postgres на VPS.

Если сайт уже работает, а нужно изменить текст, блок, ссылку, SEO, FAQ или добавить страницу, **не выполняйте команды из этого файла**. Идите в Strapi:

```text
https://strapi.integromat.ru/admin
Content Manager -> Page
```

Для обычной работы редактора читайте:
...
Этот файл нужен для самого простого первого запуска на чистой Ubuntu VPS или для аварийного восстановления.
...
Важно: следующую команду можно запускать **только из корня проекта на сервере**, обычно это:

```bash
cd /srv/chatplus
```

Если вы сейчас в `/root`, команда не найдётся и вы почти наверняка делаете не то.
...
Первый запуск на чистой Ubuntu или аварийное восстановление инфраструктуры:
