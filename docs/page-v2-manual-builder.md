...
### Важное правило для preview старых семейств

Для страниц семей `entity_detail` и `entity_intersection` preview и публичный сайт рендерятся не как свободный список секций, а через структурный legacy-макет.

Это значит:

- hero берется только из первого hero-слота страницы;
- если в `sections` добавить еще один `hero` ниже по странице, он не появится в preview и на live-странице;
- порядок секций для таких страниц должен повторять существующий макет, а не превращаться в произвольный набор блоков.

Если нужно поменять верхний hero, редактируйте первый `hero` в `sections`. Если нужен другой макет, это уже отдельная задача на новый family / renderer, а не просто добавление второго hero-блока.
...
AI не создает другой тип страниц. Он заполняет тот же `page_v2`:
...
Для новых страниц и крупных правок лучше сначала открыть нужную `Page`, нажать `Сгенерировать через AI`, проверить созданный draft в preview и только потом публиковать. Для доработки существующего AI-черновика снова запускайте AI с этой же `Page`: Strapi сам создаст связанную `Generation Job`, сохранит `target_page`, route и применит новый prompt к текущему черновику.

Главное правило: AI не должен автоматически включать `migration_ready` для старых маршрутов.
