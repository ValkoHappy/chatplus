...
- `target_blueprint` - должен совпадать с `page_kind` выбранной страницы. Простая расшифровка: `campaign` = продающая страница, `brand` = направление или категория, `resource` = гайд или инструкция.
