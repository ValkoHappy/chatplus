...
applyDescriptions(
  ...
  'Задание для AI. Создаёт или обновляет черновик Page, но не публикует страницу само.',
  {
    ...
    target_blueprint: 'Какой тип выбранной Page заполняет AI. Должен совпадать с page_kind у target_page. Пример: campaign.',
    block_strategy: 'Как AI заполняет существующий каркас: auto - сохранить блоки выбранной Page, blueprint_default - следовать blueprint выбранной Page, custom - использовать target_blocks только как подсказку.',
    target_blocks: 'Подсказка для custom-режима. AI не создает новые типы блоков, а заполняет только блоки, которые уже есть в target_page.',
    ...
    target_page: 'Обязательное поле: выберите Page, которую AI должен заполнить или доработать. Для нового URL сначала создайте Page в Content Manager -> Page.',
    ...
    generated_draft: 'AI-кандидат черновика. Это предварительный вариант до нажатия «Принять в Page».',
    applied_at: 'Время, когда AI-кандидат был принят и записан в Page.',
  },
);
