...
Роль для Алины и обычного редактора в Strapi: `AI Draft Editor`. Она разрешает работать с `Page`: создавать, редактировать, публиковать, снимать с публикации и удалять тестовые или согласованные страницы. Также она разрешает создавать `Generation Job` для AI-доработки выбранной `target_page`. Эта роль не предназначена для VPS, Docker, GitHub Actions, `.env`, API tokens, схем Strapi и серверных snapshot.
